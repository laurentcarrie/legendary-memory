...
let generate_shlib buildroot =
  let generate_one filename data =
    let filename = buildroot ^ "/" ^ filename in
    Log.info "writing sh file %s" filename;
    Out_channel.with_open_text filename (fun t ->
        Out_channel.output_string t data)
  in
  List.iter
    ~f:(fun (x, y) -> generate_one x y)
    [
      ("make_lytex.sh", Sh_code.make_make_lytex);
      ("make_mpost.sh", Sh_code.make_make_mpost);
      ("make_pdf.sh", Sh_code.make_make_pdf);
      ("make_wav.sh", Sh_code.make_make_wav);
      ("make_clean.sh", Sh_code.make_make_clean);
    ]
