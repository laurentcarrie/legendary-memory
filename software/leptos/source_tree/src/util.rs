...
async fn get_omake_progress() -> Result<(String, String)> {
    ...
    match data.choice {
        EChoice::ItemSeeProgress(data) => Ok(("omake.stdout".to_string(), serde_json::to_string(&data).unwrap())),
        _ => Ok((
            "omake.stdout".to_string(),
            format!("{}:{}, bad type", file!(), line!()),
        )),
...
pub async fn get_something_to_see(what: WhatToShow) -> Result<WhatToShowResult> {
    match what {
        ...
        WhatToShow::SourceFile(path) => Ok(WhatToShowResult::SourceFile(get_file(path))),
