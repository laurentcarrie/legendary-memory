...
pub fn App() -> impl IntoView {
    ...
    let (file_value, set_file_value) = signal::<String>(serde_json::to_string(WhatToShow::Nothing)) ;
    ...
    let async_file_data = LocalResource::new(move || get_something_to_see(
        serde_json::from_str::<WhatToShow>(file_value.get())).await.unwrap());
                                ...
                                let _ = {
                                    match &items.get(0)  {
                                        ...
                                        Some(item) => {
                                            ...
                                            set_file_value.set(
                                                serde_json::to_string(&WhatToShow::SourceFile(item.masterjsonfile.clone())).unwrap()
                                            ) ;
                                            ()
                                        }
                                    }
