...
use crate::actions::build_wav::build_wav;
...
fn compute_digest(song: &M::Song) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let mut lyrics_files = song
        ...
        .filter_map(|section| match &section.item {
            M::StructureItemContent::ItemChords(chords) => Some(chords.section_id.clone()),
            M::StructureItemContent::ItemHRule(_) => None,
            M::StructureItemContent::ItemNewColumn => None,
            M::StructureItemContent::ItemRef(xref) => Some(xref.section_id.clone()),
        })
        .map(|id| {
...
fn pathbuf_ok_checksum(song: &M::Song) -> PathBuf {
    let mut p = PathBuf::from(&song.builddir);
    p.push(".checksum_ok");
    p
}
// fn pathbuf_failed_checksum(song: & M::Song) -> PathBuf {
//     let mut p = PathBuf::from(&song.builddir);
...
fn needs_rebuild_ok(world: &M::World, song: &M::Song) -> bool {
...
// fn needs_rebuild_failed(song: & M::Song) -> Result<bool, Box<dyn std::error::Error>>  {
...
/// build a pdf for a song, and also wav files if any
...
pub async fn wrapped_build_pdf_book(tx: Sender<M::LogItem>, world: M::World, book: M::Book) {
...
pub async fn build_pdf_song(
    ...
    if !needs_rebuild_ok(&world, &song) {
        ...
        let li = M::LogItem::Song(M::LogItemSong {
            author: song.author.clone(),
            title: song.title.clone(),
            status: M::ELogType::NoNeedSuccess,
        });
        tx.send(li).await?;
        return Ok(());
    }
    // if needs_rebuild_failed(&song)? {
    //     let li = LogItem::Song(M::LogItemSong {
    //         author: song.author.clone(),
    //         title: song.title.clone(),
    //         status:  M::ELogType::NoNeedFailed,
    //     });
    ...
    for wavfile in &song.wavfiles {
        let li = M::LogItem::Song(M::LogItemSong {
            author: song.author.clone(),
            title: song.title.clone(),
            status: M::ELogType::Wav(wavfile.clone()),
        });
        tx.send(li).await?;
        match build_wav(song.clone(), wavfile.clone()).await {
            Ok(()) => (),
            Err(e) => {
                log::error!("{e:?}");
                success = false
    ...
    loop {
        ...
        let li = M::LogItem::Song(M::LogItemSong {
            author: song.author.clone(),
            title: song.title.clone(),
            status: M::ELogType::Lualatex(count as u32),
        });
        tx.send(li).await?;
        ...
        if !(output.status.success()) {
            ...
            //         format!("lualatex failed for {} {}", &song.author, &song.title),
            //     ),
            // ));
            break;
        }
...
async fn build_pdf_book(
    ...
    let li = M::LogItem::Book(M::LogItemBook {
