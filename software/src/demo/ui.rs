...
pub fn drawx<B: Backend>(
    f: &mut Frame<B>,
    logsrunning: Vec<ListItem<'_>>,
    logsdone: Vec<ListItem<'_>>,
) {
    ...
    draw_chartsx(f, logsrunning, logsdone, chunks[0]);
}
...
fn draw_chartsx<B>(
    f: &mut Frame<B>,
    logsrunning: Vec<ListItem>,
    logsdone: Vec<ListItem>,
    area: Rect,
) where
    B: Backend,
{
        ...
        .split(area);
    {
            ...
            .split(chunks[0]);
        {
            ...
            let logsrunning =
                List::new(logsrunning).block(Block::default().borders(Borders::ALL).title("Logs"));
            let mut state = ListState::default();
            f.render_stateful_widget(logsrunning, chunks[0], &mut state);

            let logsdone =
                List::new(logsdone).block(Block::default().borders(Borders::ALL).title("Done"));
            let mut state = ListState::default();
            f.render_stateful_widget(logsdone, chunks[1], &mut state);
        }
    }
}
    ...
    B: Backend,
{
        ...
        .split(area);
    {
            ...
            .split(chunks[0]);
        {
            ...
            let author_style = Style::default().fg(Color::Blue);
            let title_style = Style::default().fg(Color::Green);
            // let title_style = Style::default().fg(Color::Magenta).bg(Color::White);
