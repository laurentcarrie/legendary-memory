...
impl Universe {
    pub fn tick(&mut self) {
        self.guess = self.guess.iter().map(|l| {
            let guess = l.guess;
            let current = l.current;
            let current = if current == guess { guess } else {
                (random() * 200.0) as u32
            };
            Letter {
                guess: guess,
        ...
        let success = self.guess.iter().fold(0u32,|acc,l| if l.guess==l.current { acc+1 } else { acc } ) ;
...
impl Universe {
    ...
    pub fn the_message(&self) -> String {
        let current = self.guess.iter().map(|i| {
            char::from_u32(i.current).unwrap()
        }).into_iter().collect();
        current
    }
...
#[wasm_bindgen]
pub fn plot_component_script_2(universe:&Universe,id:String) -> String {

    log(format!("in rust, id = {id}",id=id).as_str()) ;

    let mut plot = Plot::new();

    let x = std::ops::Range{start:0,end:universe.attempts.len()}.collect();
